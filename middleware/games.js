...
const CheckIsGameExist = async(req, res, next)=>{
  const isExist = req.gamesArray.find((game)=> {return req.body.title===game.title});
  if (isExist){
    res.setHeader("Content-Type", "application/json");
    res.status(400).send(JSON.stringify({message: "Такаая игра уже существует"}));
  }else{
    next()
  }
};
...
module.exports = {findAllGames, createGame, findGameById, updateGame, deleteGame, checkEmptyFields, checkIfCategoriesAvailable, checkIfUsersAreSafe, CheckIsGameExist, checkIsVoteRequest};